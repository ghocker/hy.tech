...
$para = $_GET["nama"];
$data_keuangan = query("SELECT * FROM keuangan WHERE asal = '$para'");
$tanggal = query("SELECT DAY(tanggal), MONTH(tanggal), YEAR(tanggal), HOUR(tanggal), MINUTE(tanggal) FROM keuangan WHERE asal = '$para'");
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="container mt-4">
            ...
            <div class="row mb-3">
                <div class="card p-0">
                    <div class="card-header bg-success" style="color:white;">
                        <?=$tanggal[$i]["DAY(tanggal)"], '/' ,$tanggal[$i]["MONTH(tanggal)"], '/' ,$tanggal[$i]["YEAR(tanggal)"],'   ' ,$tanggal[$i]["HOUR(tanggal)"],':' ,$tanggal[$i]["MINUTE(tanggal)"],' WIB'?>
                    </div>
                    <div class="card-body bg-light">
                        <p class="card-text p-0 m-0">Barang <span
                                style="display:inline-block; width: 98px;"></span>:&nbsp;&nbsp;&nbsp;<?=$row["barang"]?>
                        </p>
                        <p class="card-text p-0 m-0">Jumlah Transkasi <span
                        ...
                        <div class="mt-3 d-flex justify-content-end">
                            <form action="" method="post">
                                ...
                                <a href="hapustransaksimitra.php?nama=<?=$row["id"]?>" class="btn btn-danger ">Hapus</a>
                                <a href="ubahtransaksimitra.php?nama=<?=$row["id"]?>" class="btn btn-warning ">Ubah</a>
                            </form>
                        </div>
                    </div>
                </div>
            ...
            <div class="row justify-content-center">
                <div class="class pt-5 text-center">
                    <a href="keuangan.php" class="btn btn-warning">Kembali</a>
                    <a href="tambahtransaksi.php?nama=<?=$para;?>" class="btn btn-success">Tambah Data Transaksi</a>
                </div>
            </div>
        </div>
